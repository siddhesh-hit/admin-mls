    ...
    if (!files) {
      ...
      if (
        ...
        if (files[0].size > maxAllowedSize) {
          ...
          setServerData((prev) => {
            const updatedMandalImages = [...prev.mandal_image];
            if (!updatedMandalImages[index]) {
              updatedMandalImages[index] = {};
            }
            updatedMandalImages[index][name] = files[0];

            return {
              ...prev,
              mandal_image: updatedMandalImages,
            };
          });
        }
      } else {
